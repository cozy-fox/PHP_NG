...
PHP_METHOD(ngx_request, method)
{
    ...
    if (r->method == NGX_HTTP_GET) {
        ...
        ZVAL_STRINGL(return_value, (char *)"POST", sizeof("POST") - 1, 1);
    }else if (r->method == NGX_HTTP_PUT) {
        ZVAL_STRINGL(return_value, (char *)"PUT", sizeof("PUT") - 1, 1);
    }else if (r->method == NGX_HTTP_HEAD) {
        ZVAL_STRINGL(return_value, (char *)"HEAD", sizeof("HEAD") - 1, 1);
    }else if (r->method == NGX_HTTP_DELETE) {
        ZVAL_STRINGL(return_value, (char *)"DELETE", sizeof("DELETE") - 1, 1);
    }
}
...
PHP_METHOD(ngx_request, script_filename)
{
    ...
    if ((r->uri.data)[r->uri.len-1] == '/'){
        ...
        ZVAL_STRINGL(return_value, (char *)tmp_script, plcf->document_root.len + r->uri.len + 9, 1);
        efree(tmp_script);
    } else {
        char *tmp_script;
        ...
        ngx_cpystrn((u_char *)tmp_script, (u_char *)plcf->document_root.data, plcf->document_root.len+1);
        strncat(tmp_script, (char *)r->uri.data, r->uri.len);
        ZVAL_STRINGL(return_value, (char *)tmp_script, plcf->document_root.len + r->uri.len, 1);
        efree(tmp_script);
    }
}
...
PHP_METHOD(ngx_request, query_string)
{
    ...
    if (r->args.len > 0){
        ...
        ZVAL_STRINGL(return_value, (char *)"", 0, 1);
    }
}
...
PHP_METHOD(ngx_request, server_addr)
{
    ...
    if (ngx_connection_local_sockaddr(r->connection, &server_address, 0) != NGX_OK) {
        ...
        ZVAL_STRINGL(return_value, (char *)server_address.data, server_address.len, 1);
    }
}
...
PHP_METHOD(ngx_request, headers)
{
    ...
    for (i = 0; /* void */; i++){
        ...
        if (ngx_strncasecmp(header[i].lowcase_key, (u_char *)"content-type", header[i].key.len) == 0){
            add_assoc_stringl(return_value, "content_type", (char *)header[i].value.data, header[i].value.len, 1);
        }else {
            add_assoc_stringl(return_value, "content_type", (char *)"", 0, 1);
        }

        if (ngx_strncasecmp(header[i].lowcase_key, (u_char *)"content-length", header[i].key.len) == 0){
            add_assoc_stringl(return_value, "content_length", (char *)header[i].value.data, header[i].value.len, 1);
        }else {
            add_assoc_stringl(return_value, "content_length", (char *)"", 0, 1);
        }

        if (ngx_strncasecmp(header[i].lowcase_key, (u_char *)"accept", header[i].key.len) == 0){
            add_assoc_stringl(return_value, "accept", (char *)header[i].value.data, header[i].value.len, 1);
        }

        if (ngx_strncasecmp(header[i].lowcase_key, (u_char *)"host", header[i].key.len) == 0){
            add_assoc_stringl(return_value, "host", (char *)header[i].value.data, header[i].value.len, 1);
        }

        if (ngx_strncasecmp(header[i].lowcase_key, (u_char *)"connection", header[i].key.len) == 0){
            add_assoc_stringl(return_value, "connection", (char *)header[i].value.data, header[i].value.len, 1);
        }

        if (ngx_strncasecmp(header[i].lowcase_key, (u_char *)"user-agent", header[i].key.len) == 0){
            add_assoc_stringl(return_value, "user_agent", (char *)header[i].value.data, header[i].value.len, 1);
        }

        if (ngx_strncasecmp(header[i].lowcase_key, (u_char *)"accept-encoding", header[i].key.len) == 0){
            add_assoc_stringl(return_value, "accept_encoding", (char *)header[i].value.data, header[i].value.len, 1);
        }

        if (ngx_strncasecmp(header[i].lowcase_key, (u_char *)"accept-language", header[i].key.len) == 0){
            add_assoc_stringl(return_value, "accept_language", (char *)header[i].value.data, header[i].value.len, 1);
        }
        
        if (ngx_strncasecmp(header[i].lowcase_key, (u_char *)"referer", header[i].key.len) == 0){
            add_assoc_stringl(return_value, "referer", (char *)header[i].value.data, header[i].value.len, 1);
        }

        if (ngx_strncasecmp(header[i].lowcase_key, (u_char *)"pragma", header[i].key.len) == 0){
            add_assoc_stringl(return_value, "pragma", (char *)header[i].value.data, header[i].value.len, 1);
        }

        if (ngx_strncasecmp(header[i].lowcase_key, (u_char *)"cache-control", header[i].key.len) == 0){
            add_assoc_stringl(return_value, "cache_control", (char *)header[i].value.data, header[i].value.len, 1);
        }

        if (ngx_strncasecmp(header[i].lowcase_key, (u_char *)"upgrade-insecure-requests", header[i].key.len) == 0){
            add_assoc_stringl(return_value, "upgrade_insecure_requests", (char *)header[i].value.data, header[i].value.len, 1);
        }

        if (ngx_strncasecmp(header[i].lowcase_key, (u_char *)"cookie", header[i].key.len) == 0){
            add_assoc_stringl(return_value, "cookie", (char *)header[i].value.data, header[i].value.len, 1);
        }
